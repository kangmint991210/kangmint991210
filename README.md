...
유치원 교사 보조 웹앱 — 놀이활동 · 보육일지 · 관찰일지 · 알림장 · 적응일지 · 상담일지 · 생활기록부 ·
발달평가 총평 · 월간평가 · 안전교육일지 · 견학계획안 · 행사계획안 (12종). React + Vite.
...
> `documents.kind` 허용 목록에 새 종류가 들어가야 저장이 됩니다
> (현재 `life`, `assess`, `monthly`, `safety`, `trip`, `event`).
...
src/
  ...
  domain/              화면을 모르는 순수 규칙 — 테스트하기 쉬운 곳
    documents.js         문서 종류 정의, 필수 입력, 폼 초기값,
                         생활기록부 수준 · 발달평가 영역 · 안전교육 주제 · 견학 단계 · 행사 항목
...
|---|---|---|---|---|
| 무료 | ₩0 | 1종 (놀이 활동) | 3회 | ✕ |
| Basic | ₩9,900/월 | **12종 전체** | 500회 | ✓ |
| Pro | ₩19,900/월 | **12종 전체** | 2,000회 | ✓ |
...
## 공휴일 (손댈 일 없음)
달력의 빨간 날은 **자동으로 갱신됩니다.** 해가 바뀌어도, 법이 바뀌어도 코드를 고칠 필요가 없습니다.

```
브라우저 ──▶ /api/holidays?year=2026 ──▶ 구글 공개 캘린더(ICS) ──▶ { "07-17": "제헌절", … }
   │                                                                      │
   └── 7일간 localStorage 에 보관 ◀───────────────────────────────────────┘
```

- **인증키가 필요 없습니다.** 그래서 키 만료로 달력이 조용히 비는 일이 없습니다.
- 받아오지 못하면 → 저장해 둔 값 → 그래도 없으면 `src/domain/holidays.js` 의 내장 표(2025~2027)로
  내려갑니다. **모르는 해는 아예 표시하지 않습니다** — 틀린 날짜를 보여 주는 것보다 낫습니다.
- 기념일(식목일·스승의날)과 근로자의 날은 뺍니다. 관공서 공휴일이 아니라 어린이집은 대개 정상 운영이고,
  빨간 날로 칠하면 "쉬는 날"로 오해하게 됩니다.

> 왜 자동화했나 — 손으로 적은 표는 실제로 틀렸습니다. 제헌절이 2026-05-11 부터 공휴일로 되살아났고
> (대통령령 제36290호), 선거일(2025-06-03, 2026-06-03)도 법정공휴일인데 빠져 있었습니다.
> 표는 법 개정을 따라갈 수 없습니다.
